...
// ESP32 standard has 16 channels
  ...
  if (channel == -1)
  {
    for (uint8_t i = 0; i < 16; i++)
    {
      if (_analog_write_channels[i].pin == -1)
      {
        ...
        channel = i;
        // todo: rewrite ledcSetup (old, removed); instead use: ledcAttach 
        // todo: remove ledcAttachPin (old, removed); can be done using ledcSetup instead! 
        /* OLD:
        ledcSetup(channel, _analog_write_channels[i].frequency, _analog_write_channels[i].resolution); //OLD API - removed!
        ledcAttachPin(pin, channel); //OLD API - removed!
        */
